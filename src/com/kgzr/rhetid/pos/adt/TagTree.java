...
public class TagTree {
    ...
    public String getBestTag(String word) {
        ...
        Double best = 0d;
        for(String tag : tags) {
            double tagProb = observations.get(word + "^" + tag)==null ? 0d : observations.get(word + "^" + tag);
            double transProb = transProbs.get(node.getTaggedWord().getTag()+"^"+tag)==null ? 0d : transProbs.get(node.getTaggedWord().getTag()+"^"+tag);

            if (tagProb != 0d && transProb != 0d) {
     ...
     * TODO: Debate olde english or not.
     * @param word the next word to be tagged.
...
//            else if (word.charAt(word.length()-1)=='e' && word.length()>1){  //To accommodate olde english
//                return new TaggedWord(word,getBestTag(word.toLowerCase().substring(0, word.length()-1)));
//            }
...
//        else if(word.charAt(word.length()-1)=='e' && word.length()>1) { //For olde English
//            return new TaggedWord(word,getBestTag(word.toLowerCase().substring(0,word.length()-1)));
//        }
        else {
            if (DEBUG)
